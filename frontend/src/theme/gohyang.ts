...
export const C = {
  ...
  blueDeep: '#14407f',
  /** 그래프 안에서만 쓰는 한 단 밝은 남색 — 기준선·옅은 밴드처럼 **본문 글자가 아닌** 면.
   *  왜 토큰인가: 그래프에 명도 단계가 필요하다고 Tailwind 기본 blue-500/600 을 끌어 쓰면
   *  같은 화면에 파랑이 두 벌(#1a4e9c 와 #155dfc) 생긴다(실측 2026-08-20 — 머리글 남색과
   *  그래프 실선이 나란히 보였다). 값은 CHORO 4단계와 같은 색이라 지도와도 한 계열이다. */
  blueMid: '#4b79bb',
...
/* ══════════ 무대(STAGE) — 정렬의 단일 기준 ══════════
   ...
   ★ 이 화면에는 **좌측 레일이 하나만** 있어야 한다.

   왜 토큰으로 못 박는가 (실측 2026-08-20, 1440px):
     씬 서사로 옮기면서 씬마다 기둥 폭을 따로 줬더니(서사 46rem · 차트 64rem · 전폭)
     기둥이 저마다 **가운데로 모이면서** 좌측 시작점이 셋으로 갈라졌다.
       S1·S2·S3·S6  좌 104   (전폭 1232)
       S4·S5·S7·S9·S10·S11·S13  좌 208   (64rem 1024)
       S8·S12       좌 352   (46rem 736)
     스크롤을 내리는 동안 글의 시작점이 104 → 208 → 104 → 208 → 352 … 로 여덟 번
     옮겨 다닌다. 한 화면씩 끊어 보면 각 씬은 멀쩡한데, 이어서 내리면 페이지가
     흔들린다 — 사용자가 「정렬이 애매해졌다」고 한 것이 이것이다.

   그래서 폭을 씬이 정하지 않는다. **무대 하나**를 페이지 전체(고지 띠·머리글·
   주메뉴·본문·바닥글)가 공유하고, 씬은 그 무대를 전폭으로 쓴다.
   짧게 읽혀야 하는 글만 무대 **왼쪽 레일에 붙은 채** MEASURE 로 줄인다 —
   가운데로 다시 모으지 않는다(모으는 순간 레일이 하나 더 생긴다).

   폭을 64rem 으로 잡은 근거:
     · 씬 파일들이 이미 넓은 블록을 전부 `max-w-5xl`(64rem)로 캡해 두었다.
       무대를 그 값에 맞추면 차트·격자가 무대를 **정확히** 채워 우측 레일도 하나가 된다.
     · 46rem 산문이 64rem 무대에서 72%를 채운다(80rem 무대에서는 60% — 오른쪽이 빈다).
     · 실사용자 평균 83.0세. 1920px 화면에서 한 줄이 1,232px 로 늘어나면 줄바꿈마다
       눈이 되돌아갈 자리를 잃는다.
   ※ 값(여백 16/24px·모바일 상한 42rem)은 기존과 같다. 새로 정한 것은 lg 이상의 폭뿐이다. */

/** 무대 — 콘텐츠 폭 lg 이상 64rem(1024px). 상자폭 67rem = 64rem + 좌우 여백 1.5rem */
export const STAGE = 'mx-auto w-full max-w-2xl px-4 lg:max-w-[67rem] lg:px-6'

/** 전환 전 폭 — 팩트체커·분석 화면이 쓰던 값. 홈이 검증되면 여기로 합친다 */
export const STAGE_WIDE = 'mx-auto w-full max-w-2xl px-4 lg:max-w-6xl lg:px-6 xl:max-w-7xl'

/** 읽는 폭 — 무대 안에서 글이 늘어날 수 있는 한계. **왼쪽 레일에 붙인다**(mx-auto 금지) */
export const MEASURE = 'max-w-[46rem]'
