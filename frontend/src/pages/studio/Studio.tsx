...
import type { StudioMedium, StudioRatio, StudioRegionSel, StudioOutput, StudioRelic } from '../../data/studio'
import { buildStudioOutput, probeStudioLLM, refineStudio } from '../../lib/studioPrompt'
...
export default function Studio() {
  ...
  /* ── 사료 후보 — ③의 고향으로 거른다 ── */
  const relicPool: StudioRelic[] = useMemo(() => {
    const all = STUDIO_PHOTOS.items
    if (region?.kind === 'old') return all.filter(it => it.oldKeys.includes(region.id))
  ...
  function back() {
    setRefined(null); setRefineFailed(false); setShowOriginal(false)
  ...
  /* 전체 복사용 평문 — 결과 화면의 다섯 구획을 한 문서로 */
  const fullText = useMemo(() => {
    if (!out) return ''
    const ko = refined?.ko ?? out.promptKo
    const en = refined?.en ?? out.promptEn
    const L: string[] = []
    L.push('■ 최종 프롬프트 (한글)', ko, '', '■ 최종 프롬프트 (영문)', en, '')
    L.push('■ 사용할 이미지 순서')
    if (out.ownPhotos) {
    ...
    if (out.scenes) {
      L.push('■ 영상 구성(장면별)')
      out.scenes.forEach((s, i) => L.push(`S${i + 1}. ${s}`))
    } else if (out.compositions) {
      L.push('■ 구도 제안')
      out.compositions.forEach((s, i) => L.push(`${i + 1}안. ${s}`))
    }
                  ...
                  return (
                    <li key={it.fileId} className={`overflow-hidden ${SURFACE.card} ${on ? 'border-[#1a4e9c] dark:border-[#7aa9e8]' : ''}`}>
                      <button
                        type="button"
                        aria-pressed={on}
                        aria-label={`${it.name} — ${on ? '선택 해제' : '참고 사료로 선택'}`}
                      ...
                      >
                        ...
                        <span className="block p-2.5">
                          ...
                          <span className={`mt-0.5 block ${TYPE.cap} ${TEXT.faint} ${PROSE}`}>
                            {RELIC_CAT_LABEL[it.category]} · 제공: {it.provider}
                          </span>
      ...
      {step === 7 && out && (
        <div className="mt-5 space-y-5">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <span className={`${TYPE.cap} ${TEXT.faint}`}>「복사」는 다섯 구획 전체를 한 문서로 복사합니다.</span>
          </div>
            ...
            <div className="mt-2 space-y-3">
              <div>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <CopyButton text={refined?.ko ?? out.promptKo} id="ko" copied={copied} onCopy={copy} />
                </div>
                <p className={`mt-1.5 whitespace-pre-wrap rounded-md bg-white p-3 ${TYPE.body} ${TEXT.ink} ${PROSE} dark:bg-[#181c22]`}>
                  {refined?.ko ?? out.promptKo}
                </p>
              </div>
              <div>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <h3 className={`${TYPE.h3} ${TEXT.ink}`}>영문 — 대부분의 생성 AI가 더 잘 알아듣습니다</h3>
                  <CopyButton text={refined?.en ?? out.promptEn} id="en" copied={copied} onCopy={copy} />
                </div>
              ...
              {llmOk && !refined && (
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  <span className={`${TYPE.cap} ${TEXT.faint} ${PROSE}`}>
                    누르시면 위 문장이 다듬기를 위해 한 번 전송됩니다 — 저장되지 않습니다.
                  </span>
                </div>
              )}
              {refined && (
                <div>
                  {/* 검증 범위를 정직하게 — validateStudio 는 숫자 부분집합·스키마·이모지·길이만 잰다.
                      지명은 프롬프트로 금지를 요청할 뿐 기계 검증이 없으므로, 하지 않은 검증을 했다고 말하지 않는다. */}
                  <p className={`${TYPE.cap} ${TEXT.live}`}>
                    AI가 문장을 다듬었습니다 — 수치는 입력에 있던 것만 썼는지 검사했습니다. 지명·표현은 아래 「원문 보기」로 직접 대조하십시오.
                  </p>
            ...
            <ol className="mt-2 space-y-1.5">
              {(out.scenes ?? out.compositions ?? []).map((s, i) => (
                <li key={i} className={`${TYPE.sub} ${TEXT.soft} ${PROSE}`}>
                  <b className={`tabular-nums ${TEXT.ink}`}>{out.scenes ? `S${i + 1}.` : `${i + 1}안.`}</b> {s}
                </li>
              ))}
            </ol>
            <div className="mt-2.5 text-right">
              <CopyButton
                text={(out.scenes ?? out.compositions ?? []).map((s, i) => `${out.scenes ? `S${i + 1}` : `${i + 1}안`}. ${s}`).join('\n')}
                id="scenes" copied={copied} onCopy={copy}
              />
            </div>
          </section>
          ...
          <section className={`${SURFACE.card} p-4`}>
            ...
            <p className={`mt-2 ${TYPE.body} ${TEXT.ink} tabular-nums ${PROSE}`}>
              {out.medium === 'video' ? `권장 길이 ${out.lengthLine}` : '사진 — 길이 없음'} · {out.sceneLine}
            </p>
          </section>
