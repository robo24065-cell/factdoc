...
function fanOutClaims(s: string): string[] {
  // 식품/제품/약만 주어 후보(성분·영양소 nutrient는 술어에 등장하므로 제외)
  const subs = findAllInText(s, 'subject', 8).filter((e) => !e.tags?.includes('nutrient') && e.tags?.some((t) => t === 'food' || t === 'folk' || t === 'supplement' || t === 'drug'))
  if (subs.length < 2) return [s]
  const pos = subs.map((e) => {
    let p = -1, surf = ''
    for (const v of variantsOf(e.canonical)) { const i = s.indexOf(v); if (i >= 0 && (p < 0 || i < p)) { p = i; surf = v } }
    return { p, surf }
  }).filter((x) => x.p >= 0).sort((a, b) => a.p - b.p)
  if (pos.length < 2) return [s]
  // 첫 '연속 나열'만 — 연결어(와/과/,/및/그리고)로 이어진 동안만 주어. 토픽 조사(는/은…)나 다른 단어가 끼면 중단(술어 시작).
  const list = [pos[0]]
  for (let i = 1; i < pos.length; i++) {
    const gap = s.slice(pos[i - 1].p + pos[i - 1].surf.length, pos[i].p)
    if (/^[\s,·•및그리고와과랑]*$/.test(gap)) list.push(pos[i]); else break
  }
  if (list.length < 2) return [s]
  const last = list[list.length - 1]
  const predicate = s.slice(last.p + last.surf.length).replace(/^[\s,·및그리고와과은는이가도들]+/, '').trim()
  if (predicate.length < 4) return [s]
  const seen = new Set<string>()
  return list.map((x) => x.surf).filter((x) => x && !seen.has(x) && (seen.add(x), true)).map((subj) => `${subj}는 ${predicate}`)
}
...
export default function Home() {
  ...
  async function check(text: string) {
    ...
    // 0·) 여러 주장이 섞인 긴 글(챗봇 답변·블로그 복붙) → 주장별로 나눠 각각 판정(인라인 [검증결과] + 펼침)
    {
      ...
      const claimParts = [...new Set(parts.filter(claimLike).flatMap(fanOutClaims))]
      if (claim.length >= 60 && parts.length >= 2 && claimParts.length >= 2) {
        ...
        const verifyOne = (s: string): MultiClaim => {
          ...
          if (j.verdict === 'unverified') {
            // 문장에 명시된 질환 > 글 전체 주제질환(docDz). 트리플 objectDisease는 '(미상)'일 수 있어 사용 안 함.
            const dz = findInText(s, 'disease')?.canonical || docDz
            ...
            // (b) 음식이 질환에 도움 주장 — 음식 KB에서 '그 질환(docDz)' 도메인 효과(전체 effects 검색, 잘림 없음)
            if (j.verdict === 'unverified' && dz && (isBeneficialClaim(s) || /좋|도움|풍부|낮추|낮아|줄이|줄여|배출|확장|이완|공급|보충|감소|탁월|개선|효과|관리/.test(s))) {
              const fe = foodEffectFor(s, dz)
              if (fe) {
                const over = /탁월|특효|완치|즉시|무조건|최고|100%|단번|확실/.test(s)
                const v: Verdict = (fe.level === 'caution' || fe.level === 'folk' || over) ? 'partial' : 'true'
                j = { ...j, verdict: v, confidence: fe.level === 'mfds' ? 0.7 : 0.5 }
                exp = `${fe.name} — ${fe.effect}${over ? ' 다만 ‘탁월·특효’ 같은 표현은 과장일 수 있어요(식품은 보조적).' : ' 식품은 보조적이며 약·표준치료를 대체하지 않아요.'}`
              }
            }
          }
          return { text: s, j, exp }
    ...
    // ★팩트체크 의도 — "A하면 B?"류 진위 질문은 백과사전(info)이 아니라 판정 경로로. 주체가 미등록(구충제·아스파탐·전자레인지 등)이어도.
    //   순수 정의/증상/예방 질문(뭐예요/증상/예방법)은 제외하고 info 유지.
    const isFactCheckQ = !isDefSymptomQ(claim) && FACTCHECK_RE.test(claim)
    if (intent.intent === 'info' && intent.disease && !isRelationalClaim && !isFactCheckQ) {
