...
import { checkStatClaim, checkFolkRemedyClaim, checkVaccineClaim, checkChronicMedMyth } from './stats'
import type { Judgement } from './types'

export function runPipeline(text: string): Judgement {
  const claim = text.trim()
  return checkVaccineClaim(claim) ?? checkChronicMedMyth(claim) ?? checkStatClaim(claim) ?? checkFolkRemedyClaim(claim) ?? judge(parseClaim(claim), claim)
}
...
export function runPipelineRulesOnly(text: string): Judgement {
  const claim = text.trim()
  return checkVaccineClaim(claim) ?? checkChronicMedMyth(claim) ?? checkStatClaim(claim) ?? checkFolkRemedyClaim(claim) ?? judge(parseClaim(claim), claim, { useGraph: false })
}
...
export { checkStatClaim, checkFolkRemedyClaim, checkVaccineClaim, checkChronicMedMyth, prevalenceFor, PREVALENCE_DISEASES } from './stats'
export type { PrevalenceInfo } from './stats'
...
export { findInText, findAllInText, suggest } from './ontology'
export type { Suggestion } from './ontology'
export { symptomsFor } from './symptoms'
export { rumorsFor } from './disease-rumors'
export { foodAnswer, foodAnswerAll, foodEffectFor, sharesDomain, foodsFor, dishCaution, isNonFood, mentionsFood } from './food'
