...
import { checkStatClaim, checkFolkRemedyClaim, checkVaccineClaim } from './stats'
import type { Judgement } from './types'

export function runPipeline(text: string): Judgement {
  const claim = text.trim()
  return checkVaccineClaim(claim) ?? checkStatClaim(claim) ?? checkFolkRemedyClaim(claim) ?? judge(parseClaim(claim), claim)
}
...
export function runPipelineRulesOnly(text: string): Judgement {
  const claim = text.trim()
  return checkVaccineClaim(claim) ?? checkStatClaim(claim) ?? checkFolkRemedyClaim(claim) ?? judge(parseClaim(claim), claim, { useGraph: false })
}
...
export { checkStatClaim, checkFolkRemedyClaim, checkVaccineClaim, prevalenceFor, PREVALENCE_DISEASES } from './stats'
