...
import { checkStatClaim, checkFolkRemedyClaim } from './stats'
import type { Judgement } from './types'

export function runPipeline(text: string): Judgement {
  const claim = text.trim()
  return checkStatClaim(claim) ?? checkFolkRemedyClaim(claim) ?? judge(parseClaim(claim), claim)
}
...
export function runPipelineRulesOnly(text: string): Judgement {
  const claim = text.trim()
  return checkStatClaim(claim) ?? checkFolkRemedyClaim(claim) ?? judge(parseClaim(claim), claim, { useGraph: false })
}
...
export { checkStatClaim, checkFolkRemedyClaim } from './stats'
