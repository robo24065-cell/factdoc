...
// 조언/허용/추천 질문 — "X질환에 Y 먹어도 되나/뭘 먹으면 좋아/어떻게 관리/낮추려면" → 정보(관리 안내)로
const ADVICE_Q = /(먹어도|마셔도|드셔도|드시면|드실|드세요|해도\s*되|먹으면\s*안|마시면\s*안|먹어도\s*될|괜찮|되나요|돼요|되요|피해야|줄여야|조심|주의|어떤\s*음식|무슨\s*음식|뭘\s*먹|뭐\s*먹|뭐\s*드시|뭘\s*드시|뭐가\s*좋|뭐\s*좋|좋은\s*음식|좋은\s*거|좋은\s*게|식단|먹지\s*말|먹어야\s*하나|먹어야\s*해|추천|낮추려면|낮추는|줄이려면|관리하려면|관리\s*어떻게|예방하려면|좋아지려면|평생\s*먹|끊어도|어떻게\s*관리|어떻게\s*해야|뭐\s*해야|관리법|관리\s*방법)/
