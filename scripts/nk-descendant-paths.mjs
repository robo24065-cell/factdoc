...
const PATHS = [
  {
    ...
    eligibilityQuote2: '신청하신 분이 사망하셨다면 자식, 형제분 등 다른 가족분이 이산가족찾기 신청을 하실 수 있습니다. (자주 묻는 질문)',
  ...
  },
  {
    ...
    note: '후손이 조부모의 유품을 이어받는 가장 직접적인 법적 통로입니다. 그런데 서식은 「각종서식관리」 게시판 16번 글에만 있고, 정작 「기증현황」 안내 페이지는 이 서식을 링크하지 않습니다.',
  },
  {
    id: 'museum-donation',
    ...
    period: '연중',
    note: `문구가 "이산가족 여러분께서"라 후손 포함 여부를 페이지만으로는 판정할 수 없다(그래서 불명). 온라인 신청 폼·이메일·접수 주소가 없고, 기증 절차 4단계는 페이지 안에 있으나 화면에 표시되지 않는다(${BUILT_AT} 확인).`,
  },
  {
    id: 'video-letter',
    title: '영상편지 제작 신청',
  ...
  },
  {
    ...
    note: '창구는 살아 있고 자격에도 세대 제한이 없다. 그러나 통일부 스스로 "이산가족 상봉 행사 등 남북 당국간 이산가족 교류는 북한의 거부로 2018년 8월 이후 중단된 상황"이라고 적고 있다. 신청은 되지만 성사를 기대할 수 있는 상태는 아니다.',
  },
  {
    id: 'exchange-subsidy',
    ...
    note: `같은 사이트 안에서 금액이 어긋납니다 — 「교류안내」와 통일부 본청 페이지는 300/600/80만원인데, 같은 사이트 「자주 묻는 질문」은 아직 200/500/50만원으로 남아 있습니다(${BUILT_AT} 양쪽 확인). 「자주 묻는 질문」을 먼저 읽으면 틀린 금액을 믿게 됩니다.`,
  },
  {
    id: 'qna-minwon',
  ...
  },
  {
    ...
    note: `이산가족 후손 전용 프로그램이 아니라 일반 국민 대상이다. 4차 접수(2026-09-07~09-20)는 ${BUILT_AT} 기준으로 아직 시작 전이다.`,
  },
  {
    id: 'story-hometown',
    ...
    legalBasis: null,
    how: [],
    note: `"만나보실 수 있도록"이라고 쓰여 있듯 보는 코너다. ${BUILT_AT} 확인 결과 본문에 기고·투고·등록 버튼이 없다. 후손이 조부모의 고향 사진을 여기에 올릴 방법은 없다. 자격 요건 자체가 게시돼 있지 않아 판정할 수 없어 「불명」으로 둔다.`,
  },
  {
    id: 'video-contest',
    ...
    legalBasis: null,
    how: [],
    note: `접수 홈페이지가 접속되지 않는다(${BUILT_AT} 확인). 공지사항 368건 전체를 훑어도 2025·2026년 공모전 공고가 없다. 후손을 위해 만들어진 단 하나의 제도가 1회로 끝나고 창구까지 사라진 상태다.`,
  },
]
...
const GAPS = [
  ...
  },
  {
    ...
    evidence: '이산가족법 제2조제1호 원문 · 이산가족정보통합시스템 전 메뉴 문구',
  },
  {
    id: 'donation-has-no-procedure',
    title: '가장 직접적인 이어받기 통로에 절차가 없다',
    fact: '「기증현황」 안내 페이지에는 수집 대상·기간·전화번호만 있다. 기증 절차 4단계(기증신청→소장여부확인→수집→정리 및 보존)는 페이지 안에 있으나 화면에 표시되지 않는다. 온라인 신청 창구도, 접수 이메일도, 우편 주소도 없다.',
    consequence: '후손이 조부모의 사진 상자를 들고도 무엇을 어떻게 해야 하는지 알 수 없다. 유일한 방법이 근무시간 전화 한 통이다.',
    evidence: '기증현황 안내 페이지',
  },
  {
    ...
    evidence: '각종서식관리 게시판과 기증현황 안내 페이지 대조',
  },
  {
    ...
    fact: null,
    factTemplate: '「기증현황」 표의 기준일은 접속한 날짜가 자동으로 찍힌다({clientDate}). 표의 수치는 고정돼 있고, 원래 문구 "({legacyAsOf})"가 페이지 안에 남아 있다. 표의 공개건수 합계 {disclosedSum}건은 기록관 실제 조회 결과 {archiveTotal}건과 {matchWord}, 기록관 자료의 최신 생산일은 {newest} 다.',
    consequence: '후손이 기증해도 공개까지 이어진 흔적이 여러 해째 보이지 않습니다. 그런데 집계 화면은 오늘 것처럼 보입니다.',
    evidence: `기증현황 안내 페이지 · 기록관 공개 사료 총건수 · 최신 생산일 (${BUILT_AT} 재확인)`,
  },
  {
    ...
    evidence: '「나의 살던 고향은」 안내 페이지 · 제4차 실태조사 기록사업 문항',
  },
  {
    id: 'youth-program-died',
    title: '후손 세대를 겨냥한 단 하나의 사업이 1회로 끝났다',
    fact: '2024년 「남북 이산가족 영상편지 공모전」(초·중·고·대학생 3개 부문)이 유일한 후손 세대 전용 사업이었다. 접수 홈페이지가 현재 접속되지 않는다. 공지사항 368건에 2025·2026년 후속 공고가 없다.',
    consequence: '후손 수요(세대 간 교류 희망 55.7%)를 향해 정부가 낸 유일한 문이 닫혔고, 그 자리를 대신하는 상시 프로그램이 없다.',
    evidence: '이산가족정보통합시스템 공지사항 2건 · 공모전 접수 홈페이지 접속 실패 확인',
  },
  {
    ...
    evidence: '통일부 주요사업(이산가족 교류기반 구축) · 이산가족법 제8조의2 · 시행령 제4조의2',
  },
  {
    id: 'faq-stale-amounts',
    title: '같은 사이트가 서로 다른 금액을 말한다',
    fact: '민간교류경비 지원금이 「교류안내」와 통일부 본청에서는 생사확인 300만원·상봉 600만원·교류지속 80만원인데, 같은 사이트 「자주 묻는 질문」에는 200만원·500만원·50만원으로 남아 있습니다.',
    consequence: '어느 쪽이 최신인지 화면이 말해 주지 않는다. 기준일 없는 안내가 만드는 전형적 오류다.',
    evidence: '교류안내 · 통일부 민간교류 지원 · 자주 묻는 질문 3자 대조',
  },
  {
    ...
    evidence: '이산가족 신청·교류 안내 페이지의 선정 기준',
  },
  {
...
const out = {
  ...
  sources: [
    ...
    { name: '이산가족정보통합시스템 — 자주 묻는 질문', url: 'https://reunion.unikorea.go.kr/reuni/home/brd/bbsatcl/faq/list_t1.do?mid=SM00000142', asOf: BUILT_AT },
  ...
  gaps: GAPS,
  meta: {
    actionableCriterion: '후손이 자기 이름으로 신청 주체가 될 수 있고(①), 접수 창구가 실제로 살아 있을 때(②) 「열려 있다」로 보았습니다. 성사 가능성은 판정에 넣지 않고 따로 적었습니다.',
    ...
    caveats: [
      '이 목록은 공개된 안내 페이지와 법령 원문만으로 만들었습니다. 통일부 내부 지침·연도별 사업계획은 열람하지 못했습니다.',
      '유전자검사의 후손 대상 연도·규모(예: 2025년 1,550명)는 통일부 공개 페이지·법령에서 확인되지 않았습니다. 확인되지 않은 수치를 채워 넣지 않았습니다.',
      '「나의 살던 고향은」의 기고 가능 여부는 페이지에 규정이 없어 불명으로 두었습니다. 기능이 없는 것은 확인했으나 정책상 금지인지는 알 수 없습니다.',
      '이북5도위원회 명예도민증의 후손 대상 여부는 확인하지 못했습니다.',
    ],
  },
}
