  ...
  const dataTable = table(COLS, [
    ...
    row([
      ...
      cell([
        ...
        P([run('합계 — ', { b: true }), run('카탈로그에 선언한 통일부 데이터셋 35종(수집 완료 34 · 후속 자료로 대체 1) 중 레코드 인덱스에 실린 것은 31종이고, 나머지 4종 가운데 2종(용어사전·남북한 언어비교)은 다른 형식으로 수집·배포돼 팩트체커가 쓴다. 적재 레코드 68,487건 · 수치 37,912건 · 엔티티 721건(빌드 2026-08-12).')]),
      ], COL_M + COL_R, { span: 2, va: VerticalAlign.TOP }),
    ]),
  ])
...
function overview() {
  return [
    ...
    P('기록의 공백. 사람이 가장 많이 그리는 고향에 기록이 가장 적게 남았는데, 이 격차를 공표 자료로 계산해 둔 자료를 찾지 못했다. 원적이 확인되는 생존자는 18,294명이고(2025-08-31 공표, 전체 35,311명의 51.8%) 그중 황해도(구)가 6,851명으로 가장 많다(37.4%). 그런데 그 고향에 남은 공식 기록은 1인당 0.121건으로 가장 적고 가장 많은 미수복강원은 1.878건이다 — 15.5배다(기록 계는 여러 자료를 합친 값이라 단일 기준일이 없고, 집계 실행일이 2026-08-15다). 이 문서에서 「밀도」는 확인 시점이 이렇게 다른 두 계열의 나눗셈이며, 뒤에 나오는 15.5배는 전부 같은 조합이다.', { num: 1 }),
...
function detail() {
  ...
  return [
    ...
    table(flowT, [
      ...
      row([cell(['③ 공백 확인'], flowT[0]), cell(['「남은 분은 가장 많고 기록은 가장 적습니다」 — 황해도(구) 1인당 0.121건, 최상위와 15.5배.'], flowT[1])]),
      ...
      row([cell(['돌아오는 고리 — ⑦로 들어온 기록이 ②의 건수를 늘리고 ③의 공백을 줄인다. 15.5배 격차를 좁히는 길은 이 고리 하나뿐이다.'], IW, { span: 2, b: true })]),
    ]),
    CAP('③의 분모는 생존자 2025-08-31 공표값이고, 분자인 기록 계는 합산값이라 단일 기준일이 없다(확인 하한 2025-05-31). 확인 시점이 다른 값을 한 문장에 묶지 않는 규약은 도해에도 적용한다.'),

    P([run('2) 화면과 기능 10종 — ', { b: true }), run('열 가지 기능이 위 일곱 걸음 위에 놓이며, 자리는 홈 화면(한 화면에 한 가지만 보이도록 나눈 13개 장면)과 분석 요약 화면 두 곳이다. 쓰는 자료는 시점의 성격이 다르다 — 등록현황 기준일 2025-08-31 · 재북 출신지 2020-03-31 · 기록 집계 실행일 2026-08-15 · 사료 수집일 2026-08-19이며 수치마다 화면에 붙는다. 오른쪽 칸은 상술 항목이다.')], { before: 24 }),
    table(funcT, [
    ...
    P([run('3) 사용 시나리오 — ', { b: true }), run('아래 두 인물은 설명을 위한 가상 인물이고 인용한 수치만 실측값이다. 후손 — 손주가 할아버지의 고향 「황해도」를 누른다. 남은 분 6,851명으로 원적 1위인데 기록은 1인당 0.121건으로 최하위라는 안내를 읽고, 사료를 넘기다 집에 있는 사진을 떠올려 기억 카드에 적고 생애기록물 수집 동의 창구로 간다. 「기록이 많은 고향」이 아니라 「내 고향이 비어 있다」는 사실이 이 사람을 움직였다. 정책 담당자 — 「기록 계승 우선순위」 1순위 평안북도(구)(순위합 4)와 「1만 명 하회 2038~2041년」을 함께 보고 다음 연도 사료 수집 대상 고향을 정한다.')], { before: 24 }),
    ...
    table(densT, [
      ...
      ...[
        ['황해도(구)', '6,851', '829', '0.121', '37.4'],
        ['평안남도(구)', '3,555', '4,499', '1.266', '19.4'],
        ['함경남도(구)', '3,147', '1,443', '0.459', '17.2'],
        ['평안북도(구)', '2,061', '825', '0.400', '11.3'],
        ['함경북도(구)', '1,086', '621', '0.572', '5.9'],
        ['미수복경기', '1,078', '979', '0.908', '5.9'],
        ['미수복강원', '516', '969', '1.878', '2.8'],
      ].map((r) => row(r.map((v, i) => cell([v], densT[i], { align: i === 0 ? AlignmentType.JUSTIFIED : AlignmentType.CENTER })))),
    ]),
    CAP('생존자는 2025-08-31 공표값이다. 기록 계(연표·보도자료·동향·개황·사료의 합)는 합산값이라 단일 기준일이 없다 — 확인 하한 2025-05-31(개황), 집계 실행 2026-08-15, 사료 수집 2026-08-19. 강원 사료 397건 중 280건은 근거 지명이 상봉 장소(금강산·장전항·갈마)뿐이라 미수복강원 행에서만 뺐고, 사료의 「함경도(구)」 태그 59건은 남·북 판정이 불가능해 두 행에 모두 걸었다.'),
    P('사람이 가장 많은 곳에 기록이 가장 적다. 최상위와 최하위가 15.5배, 동향과 개황을 뺀 보수적 집계(연표·보도자료·사료만)로도 19배이며 순위는 그대로다. 원적 1위 황해도(구)는 원적 확인 생존자 18,294명의 37.4%인데, 7개 고향 중 하나로 지역이 판정된 남북관계연표 1,161건 가운데 16건(1.4%) · 같은 기준 보도자료 587건 가운데 14건(2.4%)뿐이고, 반대로 미수복경기(개성)는 생존자 5.9%인데 연표 33.1% · 보도자료 43.3%다. 기록은 사람이 아니라 사건을 따라 쌓였다 — 평양은 수도, 개성은 공단, 금강산은 상봉 장소였기 때문이다. 어느 고향이 더 중요하다는 뜻이 아니라 그 비대칭을 보이는 계산이다.', { before: 45 }),

    P([run('5) 기록 계승 우선순위 — ', { b: true }), run('줄어드는 속도·기록 공백·식별 가능한 사료의 공백, 세 지표의 순위를 합산해 어디부터 이어 적어야 하는지를 산출한다. 1순위 평안북도(구) 순위합 4(생존 -53.1% · 기록 0.400건/인 · 식별사료 0.018건/인), 2순위 황해도(구) 9, 가장 큰 곳은 미수복강원 18이다. 한 지표에 매달린 순위인지 자체 검사해 공개한다 — 지표를 하나씩 빼면 셋 중 둘에서 1위가 유지되고 감소 지표를 빼면 황해도(구)가 1위다. 표본이 7개뿐이라 점수가 아니라 정렬을 돕는 값이라고 화면에 적는다.')], { before: 24 }),
    ...
    P([run('17) 검증 — ', { b: true }), run('측정한 것만 말한다. 회귀 평가 48/48(안전 규칙 위반 0 · 근거 미제시 0), 실사용 난문 108/108(출처 누락 0 · 시점 누락 0), 화면 검증 72/72(48px 미만 0 · 기증자 성함 193명 전수 대조 포함), 고향 안내인 29/29를 통과한다(전부 2026-08-20 실행). 데이터 정합도 검사한다 — 생존자 5개 분류의 합이 전부 33,272이고 33,272 + 101,274 = 134,546이 맞으며, 교류현황 38개 연도행 15개 열이 공표 합계와 일치한다. 이 문서의 규격도 검사 스크립트로 확인한다.')], { before: 24 }),
  ]
}
...
function etc() {
  return [
    P([run('생성형 AI 활용 내용 — ', { b: true }), run('Gemini를 두 곳에만 쓴다. 첫째는 자연어 질문의 해석으로, 규칙이 먼저 처리하고 못 잡은 것만 넘기며 미리 정한 출력 형식 밖이면 폐기해 규칙 결과로 되돌린다. 둘째는 분석 요약 화면의 문장 정리로, 빌드 때 한 번 9문장을 생성하고 원본 분석 파일의 지문값을 함께 기록해 어느 판을 요약했는지 고정했다. 두 곳 모두 AI가 수치와 판정을 만들지 않는다. 요약에 쓰인 수치 27개는 전량 원본 분석 카드로 자동 역추적했고, 검사 14종을 통과한 문장만 화면에 올린다. 네트워크가 끊겨도 규칙만으로 동작한다.')]),
    P('본 기획서와 시제품 코드의 작성에는 Anthropic Claude를 사용했다. 이 문서의 모든 수치는 저장소의 산출물 파일에서 나왔고 수치마다 출처 파일과 기준일을 댈 수 있다.', { before: 45 }),

    P([run('참고 자료 — ', { b: true }), run('통일부 「2024년 제4차 남북 이산가족 실태조사 결과」(2025-01-24) · 이산가족정보통합시스템 게시판 공표 자료(국가통계 승인번호 제103003호) · 공공데이터포털 통일부 개방 데이터 · 남북이산가족 디지털박물관 · 「남북 이산가족 생사확인 및 교류 촉진에 관한 법률」 제2조 · 통계청 완전생명표 2024년(KOSIS DT_1B42) · 통일의식조사(서울대)')], { before: 45 }),
