  ...
  const dataTable = table(COLS, [
    ...
    row([
      ...
      cell([
        ...
        P(`이산가족정보통합시스템 12개 코너 ${nfmt(REUNION_TOTAL)}건(수집일 ${F.collectedAt}) — 코너마다 세는 단위가 다르다(사료·사진·영상 건수, 통계표 행, 입구 링크, 질의 수). 사이트가 총건수를 표시하지 않으면 「미표시」다`, { num: 1 }),
        table(REU_T, [
          row([hdD('코너', REU_T[0]), hdD('표시/수집', REU_T[1]), hdD('쓰임', REU_T[2]), spacer(REU_T[3]),
            hdD('코너', REU_T[4]), hdD('표시/수집', REU_T[5]), hdD('쓰임', REU_T[6])]),
          ...Array.from({ length: 6 }, (_, i) => row([
            ...cornerCells(REUNION_CORNERS[i], 0), spacer(REU_T[3]),
            ...cornerCells(REUNION_CORNERS[i + 6], 4),
          ])),
        ]),
...
function detail() {
  ...
  return [
    ...
    table(funcT, [
      row([hd('기능', funcT[0]), hd('하는 일', funcT[1]), hd('상술', funcT[2])]),
      row([cell(['고향별 공백 분석'], funcT[0]), cell(['기록 밀도를 순위로 낸다'], funcT[1]), cell(['나-4'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['기록 계승 우선순위'], funcT[0]), cell(['세 지표를 합쳐 차례를 정한다'], funcT[1]), cell(['나-5'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['기록 골든타임'], funcT[0]), cell(['98개월 실측에 추계를 잇는다'], funcT[1]), cell(['나-6'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['고향 좌표계 지도'], funcT[0]), cell(['두 좌표계를 한 지도에 겹친다'], funcT[1]), cell(['나-7'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['사료 탐색'], funcT[0]), cell(['고향이 붙은 1,445건을 연다'], funcT[1]), cell(['나-7'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['세대 계승 경로'], funcT[0]), cell(['경로 12종의 자격을 가린다'], funcT[1]), cell(['나-8'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['고향 안내인'], funcT[0]), cell(['규칙이 답하고 AI는 해석한다'], funcT[1]), cell(['다-10'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['기억 카드'], funcT[0]), cell(['우리 집 기억을 한 장에 적는다'], funcT[1]), cell(['다-11'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['기증 연결'], funcT[0]), cell(['기증 2경로를 맨 앞에 둔다'], funcT[1]), cell(['나-8'], funcT[2], { align: AlignmentType.CENTER })]),
      row([cell(['기준일 표시'], funcT[0]), cell(['● ▲ ■ 세 상태를 붙인다'], funcT[1]), cell(['라-12'], funcT[2], { align: AlignmentType.CENTER })]),
    ]),
    ...
    P([run('7) 고향 좌표계 지도와 사료 — ', { b: true }), run('이산가족 원적(광복 당시 7종)과 탈북민 재북 출신지(현행 13종)를 한 지도에 두 좌표계로 겹치고, 지역을 누르면 종료 공지·날씨·생존자·출신지 분포·기록 건수·사료가 한 패널에 모인다. 사료 4,342건 중 1,445건(33.3%)에 고향을 붙였고, 이미지는 저장하지 않고 원문으로 연결한다(사유는 다-11). 한계도 적는다 — 원적 「기타」 16,331명(49.1%, 2026-05-31)은 분모로 쓸 수 없다. 생산연도가 판독된 사료 3,098건 중 2,832건(91.4%)이 2000~2018년, 1946~1953년은 30건(1.0%), 2019년 이후는 0건이다 — 기록은 헤어질 때가 아니라 만날 때 만들어졌다(다만 생산연도가 없는 1,244건이 이 분포에서 빠졌고, 2019년 이후 0건은 수집이 그때 멈춘 탓일 수 있어 단정하지 않는다).')], { before: 24 }),
    ...
    P([run('10) 화면 구현 · 접근성 · 서비스 방법 — ', { b: true }), run(`화면은 React·TypeScript 정적 웹이고 갱신은 월 1회 배치다. 새 데이터베이스 없이 공표 자료를 읽어 쓰며, 수집·검증·이 문서 생성까지 전부 재실행 가능한 스크립트다. 생존 신청자 평균 나이가 83.02세(2025-08-31 확인값)라 같은 데이터를 「한눈에」와 「한걸음씩」 두 밀도로 낸다. 고향 안내인은 규칙이 먼저 답하고 AI는 해석만 담당하며 미리 정한 출력 형식을 벗어난 것은 폐기한다(검사 ${GUIDE.passed}건 통과). 팩트체커는 통합 인덱스 전량(31종 68,487건, 빌드 2026-08-12)에 질문을 대조하는 별도 화면이고, 앞의 「나」 26종 13,915건은 그중 팩트체커만 쓰는 몫이다. 지도·추계·순위 계산에는 관여하지 않는다. 접근성도 실측으로 고쳤다 — 누르는 자리 165개가 전부 48px 이상임을 자동 검사로 확인하고(2026-08-20 실측, 48px 미만 0건) 본문 대비는 2.63:1에서 4.54:1로 올렸다. 홈 화면은 한 화면에 한 가지만 보이도록 13개 장면으로 나누며, 정보를 삭제하지 않고 배치로 밀도를 조절한다(1280폭에서 홈 전체 높이 13,430px, 2026-08-20 실측).`)], { before: 24 }),

    P([run('11) 개인정보 · 저작권 · 자료 이용 조건 — ', { b: true }), run(`개인 단위 데이터를 다루지 않는다. 쓰는 것은 전부 공표 집계와 공개 사료이고 개인을 식별하거나 예측하는 계산은 설계에 없다. 기억 카드는 후손이 자기 집 기록을 한 장으로 적어 그림·인쇄로 가져가는 기능이며, 적은 내용은 서버로 보내지 않고 그 기기의 브라우저에만 임시로 남는다 — 화면에 그 사실과 「이 기기에서 지우기」 단추를 함께 둔다. 사료 이미지는 저장하지 않는다 — 기증자 저작물인데 개방형 라이선스 표기를 확인하지 못했기 때문이며, 주소만 참조하고 자세히 보기는 박물관으로 보낸다. 타 기관 자료는 그 기관이 요구하는 표기를 그대로 적는다 — 통일의식조사는 「서울대학교 통일평화연구원에서 실시한 통일의식조사 자료임.」을 싣고, 지도 지오메트리는 Natural Earth(Public Domain)임을 밝히며, 이산가족 공표 통계에는 국가통계 승인번호 제103003호를 병기한다.`)], { before: 24 }),
    ...
    P('1단계 시범(1년) — 고향별 화면을 공개하고 이산가족 정기 안내 발송에 링크를 동봉한다. 통일교육원·학교 실습 자료로 쓰고 이북5도위원회·지자체 행사에서 고향별 사료를 상영한다. 끝나면 사료 고향 태깅률 33.3% → 60%, 계승 1·2순위 고향 사료 집중 수집, 기준일 표기율 100%가 확인된다. 2단계 접수 서식 개선 — 기증 서식에 고향란 한 칸, 신청 서식에 세대 구분 한 칸. 그 두 칸이 전부이며, 끝나면 신규 기증분 고향 태깅 100%와 후손 참여 규모의 최초 집계가 가능해진다. 3단계 내부 운영 전환 — 같은 도메인에서 사료 이미지를 표시하고 원본 통계를 한 곳으로 모아 채널 간 9개월 차이를 없앤다. 별도 예산·인력 소요는 숫자로 적지 않았다 — 근거로 댈 산출물이 없기 때문이다.', { num: 1 }),

    P([run('16) 기대효과 — ', { b: true }), run(`본 사람이 이어 적는 사람이 된다. 알린다(8촌 이내 친족도 당사자이고 경로가 10종) → 모은다(통계·연표·보도자료·사료·날씨를 고향 이름 하나로) → 불러낸다(남의 집 사진이 내 집 사진을 떠올리게 한다) → 받는다(그 자리에서 기증·유전자검사·영상편지로). 들어온 기록은 다시 그 고향 화면의 재료가 되어 ${F.gapX}배 밀도 격차를 좁힌다. 정책에는 「1만 명 하회 2038~2041년」과 「계승 1순위 ${F.prio1}」라는, 예산과 순서를 논의할 숫자를 놓는다. 이산가족에게는 1순위 요청 사업(59.9%)에 답하고 후손에게는 없던 경로를 만든다. 정량 목표는 확인 방법이 있는 것만 세운다 — 15)의 각 단계에서 「끝나면 확인되는 것」이 그대로 목표이고, 사료 고향 태깅률은 지역명이 확인되는 건수 ÷ 전체 4,342건으로 배치마다 다시 계산된다. 이용자 수·유입·기증 건수 목표는 넣지 않았다 — 근거로 댈 실측이 없어 목표로 세우는 순간 지어낸 숫자가 된다.`)], { before: 24 }),
    P('교류가 멈춘 것과 이산가족 정책이 멈춘 것은 다르다 — 당국 차원 교류는 2018년 462건·2,829명을 끝으로 2026-05-31까지 89개월 연속 0건이지만, 보도자료 언급은 2010~2018년 연평균 14건에서 2019~2024년 14.7건으로 줄지 않았다(확인 하한 2025-10-24 · 2025년 제외). 성사는 남측만으로 결정되지 않으므로 이 서비스는 실적이 아니라 기록으로 할 수 있는 일에 집중하며, 같은 구조는 국군포로·납북자로 넓힐 수 있다.', { before: 45 }),

    P([run('17) 검증 — ', { b: true }), run(`측정한 것만 말한다. 회귀 평가 48/48(안전 규칙 위반 0 · 근거 미제시 0), 실사용 난문 108/108(출처 누락 0 · 시점 누락 0), 화면 검증 72/72(48px 미만 0 · 기증자 성함 193명 전수 대조 포함), 고향 안내인 ${GUIDE.passed}/${GUIDE.total}을 통과한다(안내인 검사 ${GUIDE.ranAt} 실행 · 나머지 2026-08-20 실행). 데이터 정합도 검사한다 — 생존자 5개 분류의 합이 전부 33,272이고 33,272 + 101,274 = 134,546이 맞으며, 교류현황 38개 연도행 15개 열이 공표 합계와 일치한다. 이 문서의 규격도 검사 스크립트로 확인한다.`)], { before: 24 }),
  ]
}
