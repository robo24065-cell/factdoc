...
const P = {
  ...
  /* ── 어휘 계층 전용 ──────────────────────────────────────
     이 둘은 **문서 인덱스에 레코드를 만들지 않는다.** 일부러 그렇다.
     낱말·뜻풀이를 문서로 색인하면 "안녕하세요를 북한말로?" 가 「인민의 안녕」에 걸리는
     그 사고가 오히려 늘어난다. 대응어 조회는 nk-lexicon.mjs 가 사전으로 처리한다.
     데이터셋 자체는 등록해 둔다 — 실제로 쓰고 있으므로 활용 목록에 남아야 한다.
