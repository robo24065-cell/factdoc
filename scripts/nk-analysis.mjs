   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    caveats: [
      ...
      '당국차원은 건/명 둘 다, 민간차원은 생사확인·서신교환·기타가 건만 공표된다(원문 unitNote). 그래서 두 계열을 같은 축에 겹치지 않았다.',
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    caveats: [
      `두 번째 값은 출처가 다른 두 공표를 잇는다 — CSV(data.go.kr 파일데이터, ${CSV_ASOF})와 HWP(이산가족정보통합시스템 게시판, ${HWP_ASOF}). 같은 통계지만 갱신 주기가 달라 직접 대조가 불가능하다(원본 validation.csvVsHwp).`,
      '사망자 누계는 신고·확인 시점 기준이다. 실제 사망일과 다를 수 있다.',
      `2018-04월에 사망자 1,164명(월 중앙값의 4.3배)이 한꺼번에 잡힌다 — 행정 일괄정리로 추정된다(카드 series-breaks). 다만 그 달은 기준선(2018-08) 이전이라 이 카드 수치에는 들어가지 않는다.`,
    ],
    asOf: HWP_ASOF,
  })
}
...
const oldTable = OLD_AXIS.map((o) => {
  ...
  const recordsAll = events + briefings + trends + overviews + museumHome
  const recordsNarrow = events + briefings + museumHome   // 동향(북한 내부매체 채록) 제외한 보수 집계
  return {
    id: o.id, name: o.name, originKey: o.originKey, modern: o.modern,
    survivors, defector, events, briefings, trends, overviews,
    museumAll, museumHome, recordsAll, recordsNarrow,
...

{
  ...
  push({
    ...
    verdict: '성립',
    method: `이산가족 원적 공표축 7종에 현행 13지역·구도명 태그를 붙여(map.json crosswalk) 지역별 기록을 합산: 남북관계연표 사건 + 통일부 보도자료 + 북한정보포털 동향 + 북한개황 문서 + 디지털박물관 사료(상봉장소만 걸린 건 제외). 생존자는 ${CSV_ASOF} 월별 원적 공표값. 나눗셈 한 번 — 추정·보간·가중 없음.`,
    n: oldTable.length,
    findings: [
      ...
      F('격차', `${r1(top.densityAll / bot.densityAll)}배`, `동향을 뺀 보수 집계로도 ${r1(byDensN[0].densityNarrow / byDensN.at(-1).densityNarrow)}배 — 순위가 바뀌지 않는다`),
    ...
    table: oldTable.map((r) => ({
      ...
      사료: r.museumHome, 사료전체: r.museumAll, 기록계: r.recordsAll,
      밀도: r3(r.densityAll), 탈북민출신: r.defector,
    })),
    caveats: [
      '기록이 많은 지역은 "고향이라서"가 아니라 "남북 사건의 무대라서" 많다 — 평양(수도)·개성(공단)·금강산(관광지구이자 상봉장). 이 카드는 그 비대칭 자체를 보여주는 것이지, 어느 지역이 더 중요하다는 뜻이 아니다.',
      '구행정구역과 현행 행정구역의 대응은 근사다(map.json crosswalk 원주). 실제 경계가 일치하지 않는다.',
      ...
      '지역 매칭은 지역명·도시명 부분일치다(region.json meta.matching). 동형이의어 오탐을 가드 정규식으로 줄였으나 완전하지 않다.',
    ],
    asOf: CSV_ASOF,
  })
}

{
  ...
  push({
    ...
    findings: [
      ...
      F('최대 괴리', `${maxGap[0].name} — 이산 ${pct(maxGap[0].survivors, totS)}% vs 탈북 ${pct(maxGap[0].defector, totD)}%`, `${r1(maxGap[0].gap)}%p 차`),
      F('두 번째 괴리', `${maxGap[1].name} — 이산 ${pct(maxGap[1].survivors, totS)}% vs 탈북 ${pct(maxGap[1].defector, totD)}%`, `${r1(maxGap[1].gap)}%p 차`),
      F('순위 상관', `ρ=${r3(rho)} (p=${r3(p)}, n=7)`, '★ 판정 불가 — n=7 로는 상관의 유무를 말할 수 없다. 위 비중 격차만 확정된 사실이다'),
    ],
    series: [
    ...
    caveats: [
      ...
      '★ 인과·설명을 주장하지 않는다. 두 분포가 다르다는 사실까지다.',
    ],
    asOf: CSV_ASOF,
  })
}
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    verdict: '성립',
    method: `사료 전량 ${total.toLocaleString()}건의 생산연도(producedOn) 문자열에서 4자리 연도를 추출해 도수분포. 형식이 '1991' '1991.09' '1991.06.27' '0000.00.00' 로 섞여 있어 정규식으로 연도만 뽑고, 뽑히지 않는 건은 세어서 따로 보고한다.`,
    n: dated,
    findings: [
      ...
      F('★ 2019년 이후', '0건', '상봉이 끊긴 뒤 생산연도가 확인되는 사료가 한 건도 없다'),
    ],
    series: [
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    findings: [
      ...
      F('추세 형태', `${monthly.length - 1 - upticks.length}/${monthly.length - 1}개월 단조 하락`, upticks.length === 0 ? '월별 흔들림 없이 한 방향으로만 움직인다' : `예외는 ${upticks.map((u) => `${u.month}(+${u.delta}%p)`).join(', ')} — 카드 series-breaks 가 잡아낸 원적 재분류 달과 정확히 같다`),
    ...
    caveats: [
      ...
      `원적 축은 2018-10과 2022-03에 재분류를 겪었다(카드 series-breaks). 합계 비중 자체는 그 영향을 거의 받지 않으나(재분류가 축 내부 이동), 개별 지역 시계열은 그 두 지점에서 끊어 읽어야 한다.`,
      `${HWP_ASOF} 게시판 공표에서는 원적 축에 「기타」가 명시적으로 ${isan.latest.survivors.byOrigin.entries.find((e) => e.label === '기타')?.n.toLocaleString()}명(${isan.latest.survivors.byOrigin.entries.find((e) => e.label === '기타')?.pct}%)으로 실려 있다 — CSV 의 잔차와 같은 구조다.`,
    ],
    asOf: CSV_ASOF,
  })
}
...
const breaks = []
{
  ...
  push({
    ...
    findings: [
      ...
      F('★ 2018-10 해석', '관계·원적 동시 재분류', '부모·미수복강원이 줄고 형제자매·3촌이상·미수복경기가 늘었다. 총계 변화는 평월 수준이라 사망이 아니라 분류 이동이다'),
      F('★ 2018-04 해석', '사망자 일괄 반영 추정', '한 달 사망 증분이 중앙값의 4배를 넘는다. 실제 사망이 그 달에 몰린 게 아니라 신고·정리가 몰린 것으로 본다'),
    ],
    series: [
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    caveats: [
      ...
      '관계 라벨은 CSV(부모/형제자매/3촌이상)와 게시판 HWP(부부·부모·자녀/형제·자매/3촌이상)가 다르게 적는다. 같은 3분류로 보고 이었다.',
    ],
    asOf: CSV_ASOF,
  })
}
...
if (exists(corpusPath)) {
  ...
  /* ── 카드 10 ───────────────────────────────────────────────────────────── */
  {
    ...
    push({
      ...
      caveats: [
        `보도자료 코퍼스는 ${dsMeta.briefing.coverageStart}~${covEnd}다(freshness=${dsMeta.briefing.freshness}). 2010년 이전은 자료 자체가 없어 비교 구간을 2010년부터로 잡았다.`,
  ...
  /* ── 카드: 남북회담 인도분야 — 데이터셋이 2018에서 멈춰 판정 불가 ─────── */
  {
    ...
    push({
      ...
      verdict: '불가',
      rejectWhy: `남북회담 데이터셋의 coverageEnd 가 ${ds.coverageEnd}(freshness=${ds.freshness})다. 2019년 이후 "회담이 없었다"인지 "자료가 갱신되지 않았다"인지 이 자료만으로 구분할 수 없다 — stale 을 frozen 으로 읽으면 안 된다.`,
      method: `남북회담 정보 ${talkRecs.length}건의 body 에서 「분야: N」을 파싱해 연도×분야 교차표를 만들었다. 계산 자체는 성립하나, 자료의 끝이 현실의 끝인지 판정할 근거가 없어 '불가'로 둔다.`,
      n: talkRecs.length,
      findings: [
        F('인도 분야 회담', `${totalH}건`, `${ys[0]}~${ys.at(-1)}`),
        F('자료의 끝', `${ds.coverageEnd} (freshness=${ds.freshness})`, '★ 이후는 「없다」가 아니라 「모른다」'),
        F('마지막 기록', `${humanitarian.filter((p) => p.y > 0).at(-1).x}년 ${humanitarian.filter((p) => p.y > 0).at(-1).y}건`),
        F('참고', '실적 기준의 종말은 교류현황으로 판정했다', 'exchange-terminus 카드는 2026-05 까지 갱신되는 자료라 0을 단정할 수 있다'),
      ],
      series: [{ key: 'talksHumanitarian', label: '남북회담 인도 분야(건)', unit: '건', points: humanitarian }],
      caveats: [
        '분야 라벨은 body 문자열에서 파싱했다. 원문 필드가 아니라 조립된 문장에서 되뽑은 값이다.',
  ...
  /* ── 카드: 동향 시계열 — 날짜 커버리지 부족 ────────────────────────────── */
  {
    ...
    push({
      ...
      method: 'nk-index.json 의 동향 레코드 factKey(nkinfo.trend.NNN)를 nkinfoTrendDates.json 과 조인해 날짜 커버리지를 실측.',
      n: trendRecs.length,
      findings: [
        F('동향 레코드', `${trendRecs.length.toLocaleString()}건`),
        F('발생일 조인 성공', `${joined.toLocaleString()}건 (${pct(joined, trendRecs.length)}%)`),
        F('커버 연도', ys.length ? `${ys[0]}~${ys.at(-1)} (${ys.length}개 연도)` : '없음', '2016년 이후 동향에는 날짜가 없다'),
        F('판정', '시계열 불가', '지역별 총량(카드 record-density-gap)에만 쓴다 — 그건 날짜가 필요 없다'),
      ],
      series: [],
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    n: perYear.length,
    findings: [
      F('겨울 > 여름', `${wins}개 연도 / ${perYear.length}개 연도`, `부호검정 양측 p = ${r3(p)} — 통상 문턱 0.05 를 넘지 못한다`),
      ...
      F('★ 판정', '약함', '방향은 7년 중 6년 일치하지만 n=7 로는 우연을 배제할 수 없다. 게다가 이 수치는 사망일이 아니라 신고·정리 시점일 수 있다'),
    ],
    series: [
    ...
    caveats: [
      '★ 결정적 한계 — 이 자료는 사망 신고·확인이 반영된 달이지 사망한 달이 아니다. 계절성이 사망의 계절성인지 행정의 계절성인지 구분할 수 없다.',
      '2018-04에 사망 1,164명이 한꺼번에 잡힌다(중앙값의 4.3배). 그 달을 뺀 계산은 하지 않았고, 대신 평균이 아니라 중앙값 지수를 썼다.',
      `완전 역년이 ${fullYears.length}개뿐이다. 표본이 늘기 전에는 이 카드를 근거로 쓰지 말 것.`,
    ],
    asOf: CSV_ASOF,
  })
}
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    n: span.length,
    findings: [
      F('상봉 있던 해', `중앙값 ${median(a)}건 (평균 ${r1(mean(a))}건, n=${a.length})`),
      F('상봉 없던 해', `중앙값 ${median(b)}건 (평균 ${r1(mean(b))}건, n=${b.length})`),
      F('비율', `${r1(median(a) / median(b))}배`),
      F('맨-휘트니 정확검정', `U=${mw.U}/${mw.maxU}, 양측 p=${r3(mw.pTwoSided)}`, `단측 p=${r3(mw.pOneSided)} — 양측으로는 문턱을 넘지 못한다`),
      F('★ 반례', `2018년 ${hist[2018] || 0}건`, '제21차 상봉이 있던 해인데 생산 사료가 가장 적다 — 최근 사료일수록 아직 기증·등록되지 않았을 수 있다'),
    ],
    series: [
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    verdict: '약함',
    method: `서울대 통일평화연구원 기초보고서(단일 출처, ${opinion.reports[0].year}~${opinion.reports.at(-1).year})의 「통일이 필요하다」 응답률과, 같은 해 8월 말 생존자 수를 겹치는 ${ys.length}개 연도에서 대조. 수준(level) 상관과 1차 차분 상관을 **둘 다** 낸다.`,
    n: ys.length,
    findings: [
      ...
      F('수준 상관', `r=${r3(rLevel)}`, '★ 이 값을 근거로 쓰면 안 된다 — 둘 다 단조 하락이라 무엇을 넣어도 높게 나온다'),
      F('1차 차분 상관', `r=${r3(rDiff)} (n=${dx.length})`, `추세를 걷어내면 동행성이 사라진다. 변화 방향 일치는 ${agree}/${dx.length}년으로 동전 던지기 수준`),
      F('★ 판정', '약함 — "같은 기간 함께 내려갔다"까지만', '인과도, 선행·후행도, 공행성도 이 자료로는 말할 수 없다'),
    ],
    series: [
    ...
    caveats: [
      `겹치는 연도가 ${ys.length}개뿐이다. 월별 등록현황 CSV 가 ${M0.month} 부터라 그 이전으로 늘릴 수 없다.`,
      '장기 계열(2007~2025)은 인포그래픽 XLSX 와 기초보고서 PDF 두 출처를 이은 것이라(중복 8개 연도에서 최대 1.4%p 차) 상관 계산에는 쓰지 않고 배경으로만 그린다.',
      '통일의식조사 표본은 매년 1,200명이다. 연도 간 1~2%p 변동은 표본오차 안일 수 있다.',
      '★ 인과 주장 금지. 두 계열이 같은 시기에 내려갔다는 관측뿐이다.',
    ],
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    verdict: '약함',
    method: `통일의식조사 「통일 이유」 문항(${ys[0]}~${ys.at(-1)}, ${ys.length}개 연도) 6개 선택지의 연도별 응답률에 단순 선형회귀를 걸어 기울기와 R² 를 비교.`,
    n: ys.length,
    findings: [
      F('이산가족의 고통 해결', `${isanRow.values[0]}% → ${isanRow.values.at(-1)}%`, `기울기 ${r3(isanRow.fit.slope)}%p/년, R²=${r2(isanRow.fit.r2)} — 추세라 부를 수 없는 수준`),
      F('구간 평균', `${r2(mean(isanRow.values))}%`, `최저 ${Math.min(...isanRow.values)}% · 최고 ${Math.max(...isanRow.values)}%`),
      ...others.slice(0, 3).map((r) => F(r.label, `${r.values[0]}% → ${r.values.at(-1)}%`, `기울기 ${r3(r.fit.slope)}%p/년, R²=${r2(r.fit.r2)}`)),
      F('★ 판정', '약함 — "줄지 않았다"까지만', '늘었다고 말할 근거도 없다(R²=0.22). 다른 이유들이 뚜렷하게 움직이는 동안 이 항목만 평평했다는 대조가 이 카드의 내용이다'),
    ],
    ...
    caveats: [
      `이 문항의 인포그래픽 계열은 ${ys.at(-1)}년에서 끊긴다. ${ys.at(-1) + 1}년 이후는 배포된 XLSX 가 없다(원본 opinion.json meta.caveats).`,
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    title: '「고향에 사람이 많으면 기록도 많은가」 — n=7로는 답할 수 없다',
    question: '고향별 생존자 수와 기록량 사이에 상관이 있는가?',
    verdict: '불가',
    rejectWhy: `단위가 구행정구역 7개뿐이다. 스피어만 정확검정에서 가장 작은 p 가 ${r3(minP)}로, 어떤 조합도 통상 문턱을 넘지 못한다. n=7 에서는 ρ=0.7 도 우연과 구분되지 않는다 — "상관이 있다"도 "없다"도 말할 수 없다.`,
    method: `7개 구행정구역에 대해 생존자 수와 기록 5종의 스피어만 순위상관을 계산하고, 7!=5,040 순열 전수로 정확 p 를 냈다(정규근사 사용 안 함).`,
    n: 7,
    findings: [
      ...pairs.map((p) => F(`생존자 × ${p.label}`, `ρ=${r3(p.rho)}`, `정확 p=${r3(p.p)}`)),
      F('★ 판정', '판정 불가', '대신 record-density-gap 카드가 상관 없이 성립하는 사실(정확한 건수와 그 비율)만으로 같은 이야기를 한다'),
    ],
    series: [{ key: 'scatter', label: '생존자(x) × 기록 총계(y)', unit: '명 / 건', points: oldTable.map((r) => ({ x: r.survivors, y: r.recordsAll, label: r.name })) }],
    caveats: [
      '지역 단위를 13개 현행 행정구역으로 늘려도 이산가족 원적 축이 7종뿐이라 조인 단위는 7로 되돌아간다.',
   ...
   ══════════════════════════════════════════════════════════════════════════ */
{
  ...
  push({
    ...
    verdict: '불가',
    rejectWhy: `저장된 관측은 지역당 관측소 1~2곳의 **단일 날짜**뿐이다(최신 ${w.latestObsDate}). 시계열도 평년값도 없어 비교·추세·계절성 어느 것도 계산할 수 없다. 2026년 자료는 NOAA 에 아직 없다(원본 meta 실측).`,
    method: '지역별 weather 배열의 관측 날짜·관측소 수를 세었다.',
    n: obs.length,
    findings: [
      ...
      F('판정', '실시간 호출로 대체', 'CLAUDE.md 결정대로 기상만은 빌드에 굽지 않고 화면이 Open-Meteo 를 직접 부른다 — 저장하는 순간 stale 이 되는 유일한 계열이다'),
    ],
    series: [],
    caveats: [w.asOfNote || '관측 기준일 주석은 원본 region.json meta.weather 참조.'],
...
if (exists(path.join(PACK, 'descendant.json'))) {
  ...
  push({
    ...
    method: 'descendant.json 의 모든 지표에서 지역 축의 존재 여부를 확인.',
...
const out = {
  ...
  asOfByLane: {
    '이산가족 월별 등록현황(CSV)': CSV_ASOF,
    '이산가족 신청·교류현황(게시판 HWP)': HWP_ASOF,
    '이산가족 교류현황': EX_ASOF,
    '이산가족 연표': CHRONO_ASOF,
  ...
  sources: [
    ...
    ...region.sources.map((s) => ({ ...s, usedBy: ['record-density-gap', 'two-homeland-maps', 'region-survivor-record-corr', 'homeland-weather'] })),
  ],
  corpus: corpusMeta,
  cards,
  meta: {
    ...
    rejected,
    stats: {
      note: '유의성은 전부 정확검정이다 — n 이 7~19라 정규근사를 쓰지 않았다.',
      methods: ['스피어만 순위상관 + 7! 전수 순열 정확 p', '맨-휘트니 U 정확검정(조합 전수)', '부호검정 정확 p', '단순 선형회귀(추세 유무 판정용, 유의성 주장 안 함)'],
    },
    principles: [
